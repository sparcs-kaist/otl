...
class TimetableTestCase(TestCase):
    ...
    def testViewTimetable(self):
        ...
        response = self.client.get('/timetable/view/', {'table_id': 1, 'view_year': 2009, 'view_semester': 1})
        result = json.loads(response.content)
        self.assertEqual(response.status_code, 200, 'Status check')
        self.assertEqual(result['result'], 'OK', 'Result message check')
        ...
        response = self.client.get('/timetable/view/', {'table_id': 2, 'view_year': 2009, 'view_semester': 1})
        result = json.loads(response.content)
        self.assertEqual(response.status_code, 200, 'Status check')
        self.assertEqual(result['result'], 'OK', 'Result message check')
        ...
        response = self.client.get('/timetable/view/', {'table_id': 3, 'view_year': 2009, 'view_semester': 1})
        result = json.loads(response.content)
        self.assertEqual(response.status_code, 200, 'Status check')
        self.assertEqual(result['result'], 'OK', 'Result message check')
